...
async def generate_pdf(data: dict) -> bytes:
    ...
    data : dict
        Keys expected:
          ...
          kitchenFinishImage (bytes | None, optional),
          kitchenFinishColors (dict[str, str], optional — 2-8 of the 8
        ...
        NOTE: the Kitchen Finish slide is entirely optional. If
        kitchenFinishImage is absent it's skipped outright; if present, the
        API route enforces 2-8 non-empty kitchenFinishColors keys — mirrors
        the frontend's validation but is also enforced server-side.
    """
...
#         if swatch_uri:
#             bg = f"background-image:url('{swatch_uri}'); background-size:cover;"
#         else:
#             # Selected but the swatch failed to resolve from S3 — leave a
#             # plain filled circle rather than a broken/blank one.
#             bg = "background:#e5ddd3;"
