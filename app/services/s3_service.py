"""S3-backed finish catalog storage."""
...
def get_finish_thumbnail(category: str, color_id: str) -> bytes:
    ...
    Cached: thumbnails are uploaded immutable, and every PDF re-fetched the same
    2-8 swatches over the network. Finish create/delete clears this — see
    clear_finish_thumbnail_cache(). Failed fetches are not cached, so a missing
    object is retried on the next render.
...
def clear_finish_thumbnail_cache() -> None:
    """Call after any change to the finish catalog: the S3 keys are derived from
    category + slug, so a delete-then-recreate reuses a key we may have cached."""
    get_finish_thumbnail.cache_clear()
