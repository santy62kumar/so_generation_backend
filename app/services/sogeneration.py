...
from ..db.models import Cabinet, CodeRaw, ColorCode
...
# ── Per-sheet state ───────────────────────────────────────────────────────────
# Everything mutated while walking one workbook lives here, not in module
# globals: two concurrent /process-xlsx requests would otherwise share
# tt_color / deferred rows / category dedupe and corrupt each other's output.
# The lookup dicts are pre-loaded once per sheet (see _load_lookups) so row
# processing does zero database round-trips.

_LOOKUP_CHUNK = 900  # keep IN (...) parameter lists well under Postgres' limit


class SheetState:
    def __init__(self, cabinets=None, colours=None, odoo_codes=None):
        self.cabinets = cabinets or {}          # cabinet_code -> Cabinet
        self.colours = colours or {}            # colour_name  -> colour_code
        self.odoo_codes = odoo_codes or {}      # infurnia_code -> odoo_code
        self.tt_color = None
        self.deferred_bom_rows = []
        self.processed_categories = set()
        self.light_cabinet_count = 0
        self.total_light_length = 0


def _chunked_lookup(db, column, values):
    """Fetch rows whose `column` is in `values`, in bounded IN (...) batches."""
    values = [v for v in values if isinstance(v, str) and v]
    rows = []
    for start in range(0, len(values), _LOOKUP_CHUNK):
        batch = values[start:start + _LOOKUP_CHUNK]
        rows.extend(db.query(column.class_).filter(column.in_(batch)).all())
    return rows


def _load_lookups(db, models, finishes) -> SheetState:
    """One query per lookup table instead of one per spreadsheet row."""
    cabinets = {}
    for cabinet in _chunked_lookup(db, Cabinet.cabinet_code, models):
        # Duplicate codes existed before; keep the first, matching the old .first().
        cabinets.setdefault(cabinet.cabinet_code, cabinet)

    colours = {}
    for colour in _chunked_lookup(db, ColorCode.colour_name, finishes):
        colours.setdefault(colour.colour_name, colour.colour_code)

    odoo_codes = {
        row.infurnia_code: row.odoo_code
        for row in _chunked_lookup(db, CodeRaw.infurnia_code, models)
    }
    return SheetState(cabinets, colours, odoo_codes)


# ── Lookups (in-memory) ───────────────────────────────────────────────────────

def get_colour_code(state, finish, model, index, reference, failed_rows):
    colour_code = state.colours.get(finish)
    if not colour_code:
        failed_rows.append({
            "Row": index + 1,
            "Model": model,
...
def get_odoo_code(state, model, index, reference, failed_rows):
    odoo_code = state.odoo_codes.get(model)
    if not odoo_code:
        failed_rows.append({
            "Row": index + 1,
            "Model": model,
...
def _flush_deferred_bom_rows(state, results):
    tt_color = state.tt_color

    for row in state.deferred_bom_rows:
        cabinet   = row["cabinet"]
        finish    = row["finish"]
    ...
    state.deferred_bom_rows.clear()


def finalize_mk_processing(state, results, failed_rows):
    if not state.deferred_bom_rows:
        return

    if state.tt_color:
        # LC appeared late — flush whatever remains
        _flush_deferred_bom_rows(state, results)
    else:
...
def process_mw_model(state, model, finish, quantity, index, reference,
    ...
    """
    cabinet = state.cabinets.get(model)
    if not cabinet:
        failed_rows.append({
    ...
    colour_code = get_colour_code(state, finish, model, index, reference, failed_rows)
    if not colour_code:
...
def process_mk_model(state, model, finish, quantity, index, reference,
                     failed_rows, results, customer_meta=None):

    # tt_color survives across rows (it is set by the first LC model in the
    # sheet), which is why it lives on `state` and is never reset per row.
    cabinet = state.cabinets.get(model)
    if not cabinet:
        failed_rows.append({
            "Row": index + 1, "Model": model,
    ...
    if "LY" in description:
        state.light_cabinet_count += 1
        match = re.search(r'-(\d+)-', description)
        if match:
            state.total_light_length += int(match.group(1)) * 2
    ...
    if is_lc_model and colour_code and state.tt_color is None:
        state.tt_color = colour_code
        # ── Flush any rows that were deferred waiting for tt_color ────────
        _flush_deferred_bom_rows(state, results)

    # ── Use colour_code if available, fall back to tt_color ──────────────────
    effective_colour = colour_code or state.tt_color
    ...
    if not effective_colour:
        ...
        state.deferred_bom_rows.append({
            "cabinet":   cabinet,
            "finish":    finish,
            "reference": reference,
            "quantity":  quantity,
...
def process_fil_model(state, model, finish, quantity, index, reference,
                      failed_rows, results, customer_meta=None):
    ...
    if category and category not in state.processed_categories:
        ...
        state.processed_categories.add(category)

    return True


def process_generic_model(state, model, quantity, index, reference,
                          failed_rows, results, customer_meta=None):
    odoo_code = get_odoo_code(state, model, index, reference, failed_rows)
    if not odoo_code:
        return False
...
def process_row(state, model, finish, quantity, index, reference,
                failed_rows, results, customer_meta, glass_shutter_found=None):
    if model.startswith(("MK-", "CK-2.0")):
        if model in _MK_FIL_MODELS:
            return process_fil_model(state, model, finish, quantity, index, reference,
                                     failed_rows, results, customer_meta)
        return process_mk_model(state, model, finish, quantity, index, reference,
                                failed_rows, results, customer_meta)

    elif model.startswith("MW"):
        return process_mw_model(state, model, finish, quantity, index, reference,
                                failed_rows, results, customer_meta,
        ...
        success = process_fil_model(state, model, finish, quantity, index, reference,
                                    failed_rows, results, customer_meta)
        if success:
            results.append({
        ...
        return process_generic_model(state, model, quantity, index, reference,
                                     failed_rows, results, customer_meta)
...
async def handle_process_xlsx(file: UploadFile, db: Session):
    ...
    try:
        contents = await read_upload_limited(file, MAX_XLSX_BYTES, "XLSX file")
        validate_xlsx_archive(contents)
    except InputValidationError as exc:
        status = 413 if "exceed" in str(exc) else 400
        raise HTTPException(status_code=status, detail=str(exc)) from exc

    # Parsing, the Odoo XML-RPC call and the workbook build are all blocking;
    # off the event loop they no longer stall every other request in flight.
    output = await asyncio.to_thread(_process_workbook, contents, db)
...
def _process_workbook(contents: bytes, db: Session) -> io.BytesIO:
    try:
    ...
    if len(df) > 20_000:
        raise HTTPException(status_code=400, detail="The workbook must not contain more than 20,000 rows.")
    ...
    try:
        ...
        try:
            project_id = bounded_text(first_line, "Project ID", 200, required=True)
        except InputValidationError as exc:
        ...
        logger.warning("Could not extract Project ID", exc_info=exc)
        raise HTTPException(status_code=400, detail="Could not read the Project ID from the file.") from exc
    ...
    # One query per lookup table for the whole sheet, instead of three per row.
    state = _load_lookups(
        db,
        # pandas 3 turns the None returned by normalize_text back into NaN in a
        # str-dtype column, so drop it here or NaN reaches the SQL IN (...) list.
        models={m for m in df["Model"].map(normalize_text).dropna() if m},
        finishes={f for f in df["Shutter_Finish"].map(normalize_text).dropna() if f},
    )
    ...
    try:
        ...
        logger.warning("Could not extract Service Charges quantity", exc_info=exc)
    ...
    logger.info("Glass-shutter models found in sheet: %s", glass_shutter_found)
    ...
    for index, row in df.iterrows():
        ...
        success = process_row(state, model, finish, quantity, index, reference,
                              failed_rows, results, customer_meta,
                              glass_shutter_found=glass_shutter_found)
    ...
    finalize_mk_processing(state, results, failed_rows)
    ...
    if prelam_pending:
        if not glass_shutter_found:
            ...
            for item in prelam_pending:
                ...
                cabinet = state.cabinets.get(mk_product)
                if cabinet:
    ...
    if service_charge_qty is not None:
        ...
        logger.info("Service charge quantity not found; skipping SR-0001 row.")

    # ── PSU row ───────────────────────────────────────────────────────────────
    cnt = state.light_cabinet_count
    if cnt >= 1:
        results.extend(get_pr_lines(cnt, state.total_light_length))

    # ── Build output workbook ─────────────────────────────────────────────────
    output = io.BytesIO()
    safe_results = [{key: excel_safe(value) for key, value in row.items()} for row in results]
    safe_failed_rows = [{key: excel_safe(value) for key, value in row.items()} for row in failed_rows]
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        if safe_results:
            pd.DataFrame(safe_results, columns=COLUMN_ORDER).to_excel(
                writer, sheet_name="Success", index=False
            )
        if safe_failed_rows:
            pd.DataFrame(safe_failed_rows).to_excel(
