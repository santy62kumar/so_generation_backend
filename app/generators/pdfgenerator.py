...
from ..services.s3_service import get_finish_thumbnail
...
def asset_uri(filename: str) -> str:
    ...
    Cached: the slide backgrounds are static, and re-reading plus re-base64ing
    them for every slide of every request was pure repeat work.
    """
    file_path = (ASSETS / filename).resolve()
    try:
        file_path.relative_to(ASSETS.resolve())
    except ValueError:
        return ""
    if not file_path.exists():
        logger.warning("Asset not found: %s — slide will render without background", filename)
        return ""
...
def _fetch_finish_swatch_uri(composite_value: Optional[str]) -> str:
    ...
    try:
        ...
        logger.warning("Could not fetch finish swatch from S3 for %s: %s", composite_value, exc)
        return ""
...
def _build_slide_1(d: dict) -> str:
    ...
    return f"""
      ...
      ">{escape_html(d['title'])} {escape_html(d['customerName'])}</div>
    """
...
def _build_kitchen_finish_slide(d: dict) -> str:
    ...
    <div style="position:absolute; top:{center_y + circle_size / 2 + 10}px;
      ...
      <div style="font-size:13px; font-weight:400;">{escape_html(finish_name)}</div>
    </div>
    """
...
def _build_contact(d: dict) -> str:
    ...
    <div style="position:absolute; top:680px; left:345px;
      ...
      {escape_html(d['relationName'])}<br/>{escape_html(d['relationPhone'])}<br/>{escape_html(d['relationEmail'])}
    </div>
    ...
    <div style="position:absolute; top:680px; left:700px;
                font-family:'Nunito Sans','Montserrat',sans-serif;
                font-size:20px; color:#fff; line-height:1.3;">
      {escape_html(d['designerName'])}<br/>{escape_html(d['designerPhone'])}<br/>{escape_html(d['designerEmail'])}
    </div>
  """
...
<body>
  <div class="slide">
    ...
    # Fonts come from install_font_routes (in-process, sub-millisecond) and every
    # image is a data URI, so there is no network left to settle: "load" renders
    # a byte-identical content stream to "networkidle" at ~50 ms/page instead of
    # ~580 ms, since networkidle always pays its 500 ms quiet window. font-display
    # is swap, so the worst case if this ever races is a fallback face, not blank
    # text.
...
async def generate_pdf(data: dict, browser) -> bytes:
    """
    ...
    """
    logger.info("Generating PDF for: %s", data["customerName"])
    ...
    data["kitchenFinishSwatches"] = await asyncio.to_thread(
        prefetch_finish_swatches, kitchen_finish_colors
    )
    ...
    try:
        ...
        for i, slide in enumerate(SLIDES):
            ...
            logger.info("Rendering slide %s/%s: %s", i + 1, len(SLIDES), asset_file or "SKIPPED")
    ...
    logger.info("PDF generated — %s KB", len(final_pdf) // 1024)
    return final_pdf
...
def generate_pdf_sync(data: dict) -> bytes:
    ...
    return run_render(lambda browser: generate_pdf(data, browser))
