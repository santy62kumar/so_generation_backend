...
from ..db.models import Cabinet, ColorCode, CodeRaw
from .auth import get_current_user
from ..core.input_validation import excel_safe
...
def _coerce_pk(cfg: Dict[str, Any], value: str):
    if cfg["pk"] != "id":
        return value
    try:
        return int(value)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail="Primary key must be a whole number.") from exc


def _validate_payload(cfg: Dict[str, Any], payload: Dict[str, Any], *, create: bool) -> Dict[str, Any]:
    model = cfg["model"]
    editable = set(cfg["columns"])
    if not create or cfg["pk"] == "id":
        editable.discard(cfg["pk"])
    ...
    for key, value in payload.items():
        ...
        limit = getattr(model, key).property.columns[0].type.length or 2_000
        if isinstance(value, str) and len(value) > limit:
            raise HTTPException(status_code=422, detail=f"{key} must not exceed {limit} characters.")
        values[key] = value
...
def list_or_search(
    ...
    q: Optional[str] = Query(
        None, max_length=200, description="Search value — matched against the table's configured search field"
    ),
...
def create_row(
    ...
    allowed_fields = _validate_payload(cfg, payload, create=True)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Could not create %s record", table)
        raise HTTPException(status_code=400, detail="Could not create record.") from exc
    db.refresh(new_row)
...
def update_row(
    ...
    row = db.query(model).filter(pk_col == _coerce_pk(cfg, pk_value)).first()
    if not row:
        raise HTTPException(status_code=404, detail="Record not found")

    values = _validate_payload(
        cfg,
        {key: value for key, value in payload.items() if key != cfg["pk"]},
        create=False,
    )
    ...
    try:
        ...
        db.rollback()
        logger.exception("Could not update %s record", table)
        raise HTTPException(status_code=400, detail="Could not update record.") from exc
    db.refresh(row)
...
def delete_row(
    ...
    pk_value = _coerce_pk(cfg, pk_value)
    ...
    try:
        ...
        db.rollback()
        logger.exception("Could not delete %s record", table)
        raise HTTPException(status_code=400, detail="Could not delete record.") from exc
...
def download_table(
    ...
    data = [
        {key: excel_safe(value) for key, value in _row_to_dict(row, cfg["columns"]).items()}
        for row in rows
    ]
